...
icon: pen
layout: post
excerpt: "How I resolved a partitioning issue on my Windows 11 system by safely removing and recreating a recovery partition using diskpart and reagentc utilities."
---
...
![A tragic screenshot from my Disk Management tool, depicting a recovery partition between NTFS and unallocated partitions.]({{ site.cdn_url }}posts/dm_screenie.png)
...
1. In order to expand my NTFS partition, we have to delete the Recovery partition. This is made fairly simple by the `reagentc` utility in Windows.
    ...
    ![Screenshot showing a Windows command prompt with "reagentc /disable" being ran]({{ site.cdn_url }}posts/reagentc_screenie.png)
...
2. Using `diskpart`, we'll now force delete the recovery partition.
    ...
    ![Screenshot showing a Windows command prompt running "diskpart" utility, with the commands "list disk", and "select disk 0".]({{ site.cdn_url }}posts/diskpart_1_screenie.png)
    ...
    ![Screenshot showing a Windows command prompt running "diskpart" utility, with the commands "list partition", "select partition 3" and "delete partition overide".]({{ site.cdn_url }}posts/diskpart_2_screenie.png)
...
3. Profit(???)
    ...
    ![Screenshot showing a Windows command prompt running "diskpart" utility, with the commands "list partition", "select partition 3" and "delete partition overide".]({{ site.cdn_url }}posts/dm_done_screenie.png)
...
4. Extend the NTFS partition so there is about 1024 MB left of unallocated space, so we can re-add the recovery partition:
    ...
    ![]({{ site.cdn_url }}posts/dm_new_screenie.png)
...
5. Now we want to create a New Simple Volume with the unallocated space.
    ...
    ![]({{ site.cdn_url }}posts/dm_dont.png)

    You can name it whatever you want, I named mine "Recovowwy" xP

    ![]({{ site.cdn_url }}posts/dm_recovowwy.png)
...
![]({{ site.cdn_url }}posts/dm_finale.png)
